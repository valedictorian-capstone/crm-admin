...
export class AccountUpdateComponent implements OnInit {
  ...
  useForm = () => {
    this.form.reset({
      ...
      province: this.account.province ? parseInt(this.account.province, 0) : undefined,
      district: this.account.district ? parseInt(this.account.district, 0) : undefined,
      birthDate: this.account.birthDate,
      gender: this.account.gender,
      avatar: this.account.avatar,
      groups: this.account.groups.map((group) => (group.id)),
    });
  ...
  useSubmit = () => {
    if (this.form.valid) {
      this.load = true;
      this.service.update({
        ...this.account, ...this.form.value, groups: this.form.value.groups.map((id) => ({ id }))
      })
        .pipe(finalize(() => {
          this.load = false;
        }))
        .subscribe(
          () => {
            swal.fire('Notification', 'Update ' + this.account.code + ' successfully!!', 'success');
            this.useDone.emit({ ...this.account, ...this.form.value, groups: this.form.value.groups.map((id) => ({ id })) });
          },
          (error) => {
  ...
  useCheck = async (name: string) => {
    ...
    control.valueChanges.subscribe(async (data) => {
      if (data !== '') {
        ...
        if (value && this.account[name] !== data) {
