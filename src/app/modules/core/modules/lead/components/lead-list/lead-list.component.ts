...
export class LeadListComponent implements OnInit {
  ...
  useFilter = () => {
    this.leadFilter = this.leads.filter((lead, i) =>
      (lead.code.toLowerCase().includes(this.search.toLowerCase()) ||
        lead.email.toLowerCase().includes(this.search.toLowerCase()) ||
        lead.fullname.toLowerCase().includes(this.search.toLowerCase()) ||
      ...
      reader.onloadend = async () => {
        ...
        if (rs.length === 0) {
          ...
          swal.fire({
            ...
            icon: 'question',
            text: 'Have ' + rs.length + ' leads true format to import',
          }).then((res) => {
            if (res.isConfirmed) {
