...
export class OpportunityListComponent implements OnInit {
  opportunitys: CustomerVM[] = [];
  groups: GroupVM[] = [];
  opportunityFilter: CustomerVM[] = [];
  min = 0;
  active = 1;
  showSearch = false;
  search = '';
  ...
  ngOnInit() {
    ...
    this.service.findAllByType('opportunity').subscribe((data) => {
  ...
  useFilter = () => {
    this.opportunityFilter = this.opportunitys.filter((opportunity, i) =>
      (opportunity.code.toLowerCase().includes(this.search.toLowerCase()) ||
        opportunity.email.toLowerCase().includes(this.search.toLowerCase()) ||
        opportunity.fullname.toLowerCase().includes(this.search.toLowerCase()) ||
      ...
      reader.onloadend = async () => {
        ...
        if (rs.length === 0) {
          ...
          swal.fire({
            ...
            icon: 'question',
            text: 'Have ' + rs.length + ' opportunitys true format to import',
          }).then((res) => {
            if (res.isConfirmed) {
              this.service.import(rs.map((e) => (
                ...
                this.opportunitys = this.opportunitys.concat(data);
                this.search = '';
                this.useFilter();
                swal.fire('Notification', 'Import ' + rs.length + ' opportunitys successfully!!', 'success');
              });
            }
          });
        }
  ...
  useQuestion = (table: ElementRef<any>, input: any) => {
    swal.fire({
      ...
      if ((res.dismiss as any) === 'cancel') {
        ...
        const wb: XLSX.WorkBook = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(wb, ws, 'Opportunity List');
        XLSX.writeFile(wb, 'example-opportunitys.xlsx');
      }
    });
  }
