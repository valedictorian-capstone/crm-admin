...
export class ActivitySavePage implements OnInit, OnChanges, OnDestroy {
    ...
    this.subscriptions.push(
      this.store.select(activitySelector.activitys)
        .pipe(
          tap((data) => {
            console.log(data);
            this.payload.activity = data.find((item) => item.id === this.payload.activity.id);
      ...
      // this.service.findById(this.payload.activity.id)
      //   .pipe(
      //     tap((data) => {
  ...
  useSubmit = (ref?: NbDialogRef<any>) => {
    ...
    if (this.state.form.valid) {
      this.useShowSpinner();
      this.subscriptions.push(
        (this.payload.activity ? this.service.update : this.service.insert)(this.state.form.value)
          .pipe(
            tap((data) => {
  ...
  useToggleDone = () => {
    this.useShowSpinner();
    this.subscriptions.push(
      this.service.update({ id: this.payload.activity.id, status: this.payload.activity.status === 'processing' ? 'done' : 'processing' } as any)
        .pipe(
          tap((data) => {
            this.toastrService.success('', 'Save activity successful!', { duration: 3000 });
            this.useDone.emit(data);
  ...
  useRemove = (ref: NbDialogRef<any>) => {
    ...
    this.useShowSpinner();
    this.subscriptions.push(
      this.service.remove(this.payload.activity.id)
        .pipe(
          tap((data) => {
            this.toastrService.success('', 'Save activity successful!', { duration: 3000 });
            this.useClose.emit();
