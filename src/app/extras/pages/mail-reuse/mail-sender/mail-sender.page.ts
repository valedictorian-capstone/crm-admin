...
interface IMailSenderPageState {
  form: FormGroup;
...
export class MailSenderPage implements OnInit, OnDestroy {
  @Output() useClose: EventEmitter<any> = new EventEmitter<any>();
  @Input() email: string;
  subscriptions: Subscription[] = [];
  state: IMailSenderPageState = {
    form: undefined,
    config: {
      editable: true,
      spellcheck: true,
  ...
  ngOnInit() {
    if (this.email) {
      this.state.form.get('email').setValue(this.email);
    }
  }
  useInitForm = () => {
