...
@Component({
  selector: 'app-support-password',
  templateUrl: './support-password.modal.html',
  styleUrls: ['./support-password.modal.scss']
})
export class SupportPasswordModal implements OnDestroy {
  ...
  useShowSpinner = () => {
    this.spinner.show('support-password');
  }
  useHideSpinner = () => {
    setTimeout(() => {
      this.spinner.hide('support-password');
    }, 1000);
  }
  ngOnDestroy() {
