...
@Component({
  ...
  templateUrl: './support-profile.modal.html',
  styleUrls: ['./support-profile.modal.scss']
})
export class SupportProfileModal implements OnInit, OnDestroy {
      ...
      const subscription = this.service.updateProfile(this.state.form.value)
        .pipe(
          tap((data) => {
            ...
            this.store.dispatch(AuthAction.UpdateProfileAction({ data }))
  ...
  useShowSpinner = () => {
    this.spinner.show('support-profile');
  }
  useHideSpinner = () => {
    setTimeout(() => {
      this.spinner.hide('support-profile');
    }, 1000);
  }
  ngOnDestroy() {
