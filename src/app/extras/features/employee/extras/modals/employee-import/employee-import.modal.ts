...
@Component({
  ...
  styleUrls: ['./employee-import.modal.scss']
})
export class EmployeeImportModal implements OnDestroy {
  ...
  useSubmit = async (ref: NbDialogRef<any>) => {
    ...
    if (this.useCheck().error === 0) {
      ...
      const success = validate.filter((e) => !e.code && !e.email && !e.phone).map((e) => this.state.formArray.find((f) => f.extra.position === e.position));
      const error = validate.filter((e) => e.code || e.email || e.phone).map((e) => this.state.formArray.find((f) => f.extra.position === e.position));
      ...
      if (this.state.formArray.length > 0) {
        this.service.import(success.map((e) => ({
          ...
          .pipe(
            tap((x) => {
              swal.fire('Success', 'Import employees succesful!', 'success');
              if (error.length > 0) {
            ...
            catchError((err) => {
              swal.fire('Fail', 'Import employees fail! ' + err.message, 'error');
        ...
        swal.fire('Invalid', 'No valid employees! Please check valid again!', 'error');
        this.state.formArray = error.map((err) => ({
          ...err,
          extra: {
            ...err.extra,
      ...
      swal.fire('Invalid', 'Some employees invalid! Please check valid again!', 'error');
    }
  }
      ...
      reader.onloadend = async () => {
        ...
        if (data.length > 1000) {
          ...
          for (let i = 0; i < data.length; i++) {
            ...
            this.useAddGroup(element as AccountVM, i);
          }
          for (let i = 0; i < this.state.formArray.length; i++) {
            const element = this.state.formArray[i];
  ...
  useAddGroup(data: AccountVM, position: number) {
  ...
  async useRemoveEmployee(index: number) {
    const rs = await swal.fire({
      title: 'Remove this employee?',
      text: 'When you click OK button, this employee will remove out of list',
  ...
  useValidBefore(position: number, name: 'phone' | 'email' | 'code') {
    const content = this.state.formArray.find((e) => e.extra.position === position);
    const control = content.data.get(name);
  ...
  useShowSpinner = () => {
    this.spinner.show('employee-import');
  }
  useHideSpinner = () => {
    this.spinner.hide('employee-import');
  }
  ngOnDestroy() {
  ...
  useFilter(type: string) {
    ...
    switch (type) {
      case 'all':
        ...
        break;
      case 'error':
        this.state.filterArray = this.state.formArray.filter((e) => e.data.invalid || e.extra.phone.error || e.extra.code.error || e.extra.email.error);
        break;
      case 'success':
        this.state.filterArray = this.state.formArray.filter((e) => e.data.valid && !e.extra.code.error && !e.extra.phone.error && !e.extra.email.error);
        break;
      default:
  ...
  useCheck() {
    return {
      error: this.state.formArray.filter((e) => e.data.invalid || e.extra.phone.error || e.extra.email.error || e.extra.code.error).length,
      success: this.state.formArray.filter((e) => e.data.valid && !e.extra.phone.error && !e.extra.email.error && !e.extra.code.error).length,
    }
  }
}
